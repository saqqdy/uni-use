...
export function external(id: string) {
	return (
		/^vue/.test(id) || /^@uni-use\/utils\//.test(id)
...
export function pathRewriter(bundlePath: string) {
	return id => {
		if (/^@uni-use\/utils\/packages/.test(id)) {
			if (noWlPrefixFile.test(id)) return id.replace('@uni-use/utils/packages/', bundlePath)
			return id.replace('@uni-use/utils/packages/', bundlePath)
		}
		if (/^@\//.test(id)) {
